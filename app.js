(function(){
  ...
  // sheet), remember it so it's attached to their signup if they create a
  // new club — even if that happens minutes or days after first clicking
  // the link, not necessarily in the same visit.
  (function captureSignupSource(){
    const params = new URLSearchParams(window.location.search);
    const ref = params.get("ref");
    if(ref){
      try{ localStorage.setItem("totem_signup_source", ref.slice(0, 60)); }catch(e){}
    }
  })();
  function getStoredSignupSource(){
  ...
  async function handleLogin(){
    ...
    if(authMode === "signup"){
      ...
      const { data, error } = await supabaseClient.auth.signUp({
        email, password,
        options: { data: signupType === "create" ? { org_name: orgName, org_type: orgType, consent_attestation: String(consentChecked), duplicate_justification: duplicateJustification, signup_source: getStoredSignupSource() } : { invite_code: inviteCode } }
      });
  ...
  function renderRoster(){
    ...
    grid.querySelectorAll(".guardian-phone-edit").forEach(inp => {
      inp.addEventListener("change", (e) => {
        const id = e.target.dataset.id;
        const p = state.players.find(pl => pl.id === id);
        if(!p) return;
  ...
  function renderSides(){
    ...
    boardEl.querySelectorAll('[data-action="whatsapp-team"]').forEach(btn => {
      btn.addEventListener("click", (e) => {
        e.stopPropagation();
        openWhatsAppShareModal(btn.dataset.sport, btn.dataset.group, btn.dataset.side, null);
      });
    });
  ...
  function renderFixtureList(){
    ...
    el.innerHTML = entries.map(entry => {
      const dateLabel = new Date(entry.date + "T00:00:00").toLocaleDateString(undefined,{weekday:"short", day:"numeric", month:"short"}) + (entry.item.time ? ` · ${formatFixtureTime(entry.item.time)}` : "");
      if(entry.kind === "trial"){
        const t = entry.item;
        return `
  ...
  function renderFixtureDetail(){
    ...
    if(openTrialId){
      ...
      const dateLabel = new Date(t.date + "T00:00:00").toLocaleDateString(undefined,{weekday:"long", day:"numeric", month:"long", year:"numeric"}) + (t.time ? ` · ${formatFixtureTime(t.time)}` : "");
    ...
    el.querySelectorAll('[data-action="whatsapp-team"]').forEach(btn => {
      btn.addEventListener("click", (e) => {
        e.stopPropagation();
        openWhatsAppShareModal(btn.dataset.sport, btn.dataset.group, btn.dataset.side, f);
      });
    });
  ...
  function normalizePhoneForWhatsApp(phone){
    ...
    if(digits.startsWith("0")) digits = "27" + digits.slice(1); // assumes South African local format
    return digits.length >= 9 ? digits : null;
  }

  function collectGuardianContacts(sportId, group, side, excludeIds){
    const sport = state.sports.find(s => s.id === sportId);
    ...
    positions.forEach(pos => {
      ...
      if(resolved && resolved.player.guardianPhone) contacts.push(resolved.player);
    });
    const benchIds = benchFor(sportId, group, side).filter(Boolean);
    benchIds.forEach(id => {
      const p = state.players.find(pl => pl.id === id);
      if(p && p.guardianPhone && !contacts.find(c => c.id === p.id)) contacts.push(p);
    });
    return contacts;
  }

  function buildTeamSheetWhatsAppText(sportId, group, side, fixture){
    const sport = state.sports.find(s => s.id === sportId);
    ...
    if(fixture){
      const dateLabel = new Date(fixture.date + "T00:00:00").toLocaleDateString(undefined,{weekday:"long", day:"numeric", month:"long"}) + (fixture.time ? ` · ${formatFixtureTime(fixture.time)}` : "");
      text += `vs ${fixture.opponent} · ${dateLabel}${fixture.venue ? " · " + fixture.venue : ""}\n`;
    }
    if(groupCoach) text += `Coach: ${groupCoach.name}\n`;
    text += `\n`;
    rows.forEach(r => { text += `${r.position}: ${r.name}\n`; });
    if(bench.length) text += `\nBench: ${bench.join(", ")}\n`;
    text += `\n\n_Build your team on Totem_ 🏆\nhttps://totem.hyperianlabs.com/?ref=whatsapp_sheet`;
    return text;
  }
  ...
  function openWhatsAppShareModal(sportId, group, side, fixture){
    ...
    const listEl = document.getElementById("whatsappShareList");
    if(contacts.length === 0){
      listEl.innerHTML = `<div style="font-size:13px; color:var(--slate);">No guardian contact numbers on file for this team yet — add one from the "Rate player" panel on the roster.</div>`;
    } else {
      listEl.innerHTML = contacts.map(p => {
        const number = normalizePhoneForWhatsApp(p.guardianPhone);
        if(!number) return `<div class="staff-row"><span class="staff-email">${escapeHtml(p.name)}</span><span style="font-size:11px; color:var(--clay);">Number doesn't look valid</span></div>`;
        const url = `https://wa.me/${number}?text=${encodeURIComponent(message)}`;
        return `<div class="staff-row"><span class="staff-email">${escapeHtml(p.name)}</span><a href="${url}" target="_blank" rel="noopener" class="btn btn-primary btn-small">Send</a></div>`;
    ...
    document.getElementById("whatsappShareModal").classList.add("open");
  }
  document.getElementById("cancelWhatsappShare").addEventListener("click", () => {
    document.getElementById("whatsappShareModal").classList.remove("open");
  });
  document.getElementById("whatsappShareModal").addEventListener("click", (e) => {
  ...
  function printAttendanceRegister(practiceId){
    ...
    win.document.write(`<!DOCTYPE html><html><head><title>Attendance — ${escapeHtml(p.ageGroups.join(", "))}</title>
      ...
      <div class="footer">Generated by Totem™${hasAttendance ? "" : " — blank register, ready to fill in"} · build your team at totem.hyperianlabs.com</div>
      </body></html>`);
    win.document.close();
    win.focus();
    win.print();
  ...
  function printTeamSheetFor(sportId, group, side, fixture){
    ...
    let subtitle;
    if(fixture){
      const dateLabel = new Date(fixture.date + "T00:00:00").toLocaleDateString(undefined,{weekday:"long", day:"numeric", month:"long", year:"numeric"}) + (fixture.time ? " · " + formatFixtureTime(fixture.time) : "");
  ...
  function openAddFixtureModal(prefillDateOrItem, editType){
    ...
    document.getElementById("fxTime").value = item ? (item.time || "") : "";
  ...
  document.getElementById("confirmFixture").addEventListener("click", () => {
    ...
    const time = document.getElementById("fxTime").value || null;
    ...
    if(type === "trial"){
      ...
      if(editingTrialId){
        ...
        trial.date = date; trial.time = time; trial.name = label; trial.venue = venue; trial.ageGroups = ageGroups;
      } else {
        state.trials.push({ id: uid(), sportId: sport.id, date, time, name: label, venue, ageGroups });
      }
    } else {
